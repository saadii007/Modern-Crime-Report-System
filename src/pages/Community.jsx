...
const Community = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-900 via-gray-900 to-black overflow-hidden">
      {/* Fixed height container */}
      <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-xl rounded-2xl w-full max-w-3xl h-[85vh] flex flex-col p-6">
        <h1 className="text-3xl font-bold text-center text-white drop-shadow-md mb-4">
          Community Forum
        </h1>

        {/* Messages area (fixed height scrollbox) */}
        <div className="flex-grow overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-blue-600 scrollbar-track-transparent">
          {messages.map((msg, index) => (
            <div key={index} className="flex items-start gap-3 hover:scale-[1.01] transition-transform">
              <img
                src={msg.avatar}
                alt="User Avatar"
              ...
              <div className="flex flex-col">
                <div className="bg-white/20 backdrop-blur-sm p-4 rounded-xl shadow-md text-white border border-white/10">
                  <p className="font-semibold text-yellow-300">{msg.username}</p>
                  <p>{msg.message}</p>
                </div>
        ...
        {/* Input stays fixed at bottom inside card */}
        <form
